...
func init() {
	inject.InjectValue("roleUserRepository_v1", roleUserRepository)
}
...
func (a *RoleUserRepository) DeleteRoleUserWithTenantId(roleId, userId, tenantId string) bool {
	// 只能删除除了 admin 之外的用户
	res := a.Model(&domain.RoleUser{}).Select("user_id").Where("role_id=? AND user_id=? AND tenant_id=?", roleId, userId, tenantId).First(&userId)
	if res.RowsAffected < 1 {
		return true
	}

	if res := a.Where("role_id=? AND user_id=? AND tenant_id=?", roleId, userId, tenantId).Delete(&domain.RoleUser{}); res.Error == nil {
		return true
	} else {
		logger.Error("RoleUserRepository 删除数据出错：", res.Error.Error())
	}
