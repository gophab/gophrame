...
type RoleService struct {
	service.BaseService
	RoleRepository *repository.RoleRepository `inject:"roleRepository_v1"`
}

var roleService *RoleService = &RoleService{}

func init() {
	logger.Debug("Initializing RoleService..._v1")
	logger.Debug("Inject roleService_v1")
	inject.InjectValue("roleService_v1", roleService)
	logger.Debug("Initialized RoleService_v1")
}
...
func (s *RoleService) CreateRole(role *domain.Role) (*domain.Role, error) {
	...
	eventbus.PublishEvent("ROLE_CREATED_v1", res)
...
func (s *RoleService) UpdateRole(role *domain.Role) (*domain.Role, error) {
	...
	eventbus.PublishEvent("ROLE_UPDATED_v1", role)
	return role, nil
}
...
func (s *RoleService) DeleteById(id string) error {
	...
	if role != nil {
		...
		eventbus.PublishEvent("ROLE_DELETED_v1", role)
	}
