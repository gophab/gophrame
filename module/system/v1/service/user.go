...
type UserService struct {
	service.BaseService
	UserRepository *repository.UserRepository `inject:"userRepository_v1"`
...
func init() {
	logger.Info("Initializing UserService..._v1")
	logger.Debug("Inject UserService_v1")
	inject.InjectValue("userService_v1", userService)
	inject.InjectValue("commonUserService_v1", commonUserService)
	_ = (service.UserService)(commonUserService)
	eventbus.RegisterEventListener("USER_LOGIN_v1", userService.onUserLogin)
	logger.Info("Initialized UserService_v1")
}
...
func (s *UserService) Create(user *dto.User) (*domain.User, error) {
	...
	eventbus.PublishEvent("USER_CREATED_v1", res)
	return res, nil
}
...
func (s *UserService) Update(user *dto.User) (*domain.User, error) {
	...
	if err = s.UserRepository.UpdateUser(exists); err == nil {
		eventbus.PublishEvent("USER_UPDATED_v1", user)
	}
