...
type SocialUserService struct {
	service.BaseService
	SocialUserRepository *repository.SocialUserRepository `inject:"socialUserRepository_v1"`
}

var socialUserService *SocialUserService = &SocialUserService{}

func init() {
	eventbus.RegisterEventListener("USER_LOGIN_v1", socialUserService.onUserLogin)
	inject.InjectValue("socialUserService_v1", socialUserService)
}
...
func (s *SocialUserService) onUserLogin(event string, args ...interface{}) {
	...
	if strings.HasPrefix(userId, "sns:") {
		...
		if socialUser, err := s.GetById(userId); err != nil || socialUser == nil {
			...
			if socialUser.UserId != nil {
				eventbus.PublishEvent("USER_LOGIN_v1", socialUser.UserId, data)
			}
