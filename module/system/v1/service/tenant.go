...
func init() {
	inject.InjectValue("tenantService_v1", tenantService)
}

type TenantService struct {
	service.BaseService
	TenantRepository *repository.TenantRepository `inject:"tenantRepository_v1"`
}
...
func (s *TenantService) Create(tenant *domain.Tenant) (*domain.Tenant, error) {
	if result, err := s.TenantRepository.CreateTenant(tenant); result != nil && err == nil {
		eventbus.PublishEvent("TENANT_CREATED_v1", tenant)
		return result, nil
	} else {
		return nil, err
	}
...
func (s *TenantService) Update(tenant *domain.Tenant) (*domain.Tenant, error) {
	if b, err := s.TenantRepository.UpdateTenant(tenant); b && err == nil {
		eventbus.PublishEvent("TENANT_UPDATED_v1", tenant)
		return tenant, nil
	} else {
		return nil, err
	}
...
func (s *TenantService) DeleteTenant(tenant *domain.Tenant) (bool, error) {
	...
	eventbus.PublishEvent("TENANT_DELETED_v1", tenant)
	return result, nil
}
