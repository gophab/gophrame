...
func (a *ModuleRepository) CreateModule(data *auth.Module) (bool, error) {
	var counts int64
	if res := a.Model(&auth.Module{}).Where("fid=? AND (title=? OR name=?)", data.Fid, data.Title, data.Name).Count(&counts); res.Error == nil && counts == 0 {
...
func (a *ModuleRepository) DeleteData(id int64) (bool, error) {
	if res := a.Delete(&auth.Module{}, id); res.Error == nil {
		// 删除下
		a.Delete(&auth.Module{}, "fids like ?", fmt.Sprintf("%%,%d,%%", id))
		return true, nil
	} else {
...
func (a *ModuleRepository) GetTenantModules(tenantId string) ([]*auth.Module, error) {
	var results = make([]*auth.Module, 0)
	if res := a.Model(&auth.Module{}).Where("tenant_id = ?", tenantId).Where("status = ?", 1).Find(&results); res.Error == nil {
...
func (a *ModuleRepository) DeleteTenantModules(tenantId string) {
	if res := a.Delete(&auth.Module{}, "tenant_id = ?", tenantId); res.Error != nil {
...
func (a *ModuleRepository) GetTenantModule(module, tenantId string) (*auth.Module, error) {
	var result auth.Module
	if res := a.Model(&auth.Module{}).Where("path = ?", module).Where("tenant_id = ?", tenantId).First(&result); res.Error == nil && res.RowsAffected > 0 {
...
func (a *ModuleRepository) makeTree(src []*auth.Module, dest *[]*auth.Module) error {
	var result = *dest
	var srcMap = make(map[int64]*auth.Module)
		...
		if item.Fid != 0 {
			...
			if parent != nil {
				if parent.Children == nil {
					parent.Children = make([]*auth.Module, 0)
				}
